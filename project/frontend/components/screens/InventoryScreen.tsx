...
export function InventoryScreen() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Kiểm Kê Kho"
        subtitle="Quản lý kiểm kê và tồn kho thực tế · Dữ liệu demo"
        actions={
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Tổng Kiểm Kê", value: inventoryData.length, color: "text-brand" },
          { label: "Hoàn Thành", value: inventoryData.filter((i) => i.status === "Completed").length, color: "text-success" },
          { label: "Đang Thực Hiện", value: inventoryData.filter((i) => i.status === "In Progress").length, color: "text-warning" },
          { label: "Chênh Lệch", value: stockData.filter((s) => s.diff !== 0).length, color: "text-danger" },
        ].map((s) => (
          ...
          <Table>
            ...
            <tbody>
              {inventoryData.map((inv) => (
                <Tr key={inv.id}>
                  <Td>
                    <span className="text-brand text-xs font-medium">{inv.id}</span>
                  </Td>
                  ...
                  <Td>
                    <span
                      className={`text-xs font-medium ${
                        inv.diff < 0 ? "text-danger" : inv.diff > 0 ? "text-warning" : "text-success"
                      }`}
                    >
                      {inv.diff > 0 ? "+" : ""}
                      {inv.diff}
                  ...
                  <Td className="text-xs text-text-muted">{inv.note || "—"}</Td>
                </Tr>
              ))}
            </tbody>
          </Table>
            ...
            <tbody>
              {stockData
                ...
                .map((s) => (
                  <Tr key={s.sku}>
                    <Td>
                      <span className="text-brand text-xs font-medium">{s.sku}</span>
                    </Td>
                    <Td className="text-xs text-text-primary">{s.name}</Td>
                    <Td>
                    ...
                    <Td>
                      <span
                        className={`text-xs font-medium ${
                          s.diff < 0 ? "text-danger" : s.diff > 0 ? "text-warning" : "text-success"
                        }`}
                      >
                        {s.diff > 0 ? "+" : ""}
                        {s.diff}
